...
of $f$, and---crucially---its variance falls as $O(1/N)$, so the error falls as $O(1/\sqrt{N})$ \emph{independently of
...
entirely when $p \propto f$. Two consequences recur. First, because error scales with $\sqrt{N}$,
...
heterogeneous medium $\tau$ has no closed form and must be estimated stochastically, for instance
by delta tracking~\cite{Woodcock1965} or its decomposition variants~\cite{Kutz2017}. A property of
...
\]
whose single parameter $g\in(-1,1)$ controls anisotropy: $g>0$ is forward scattering (clouds are
strongly forward, $g\approx 0.85$), $g=0$ is isotropic (e.g.\ a thin, optically simple haze), and
$g<0$ is backward (as in some atmospheric aerosols). $p_{\mathrm{HG}}$ is
...
\]
a weighted combination that trusts each strategy in proportion to how likely it was to have produced
the sample, and that provably keeps the combined estimator's variance close to that of the best
individual strategy.
...
extinction field is modelled as a weighted sum of $K$ Gaussian kernels, in general anisotropic,
\[
  \sigma_t(\bm{x}) = \sum_{k=1}^{K} w_k\,
...
extent and orientation), and a weight $w_k$ (its density). Such mixtures are fit to a reference
volume by expectation--maximisation or, in the radiance-field setting, by gradient descent against
images; the primitives are the same anisotropic 3D Gaussians introduced by 3D Gaussian
