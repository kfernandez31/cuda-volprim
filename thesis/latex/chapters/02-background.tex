...
\begin{figure}[htp]
  ...
  sample mean of $f/p$. \emph{Left:} one running estimate settles onto the true value as samples
  accumulate, inside the shrinking $\pm1.96\,\sigma/\sqrt{n}$ band (a 95\,\% CLT confidence interval).
  \emph{Right:} the RMS error over
  \num{256} independent runs falls along a slope-$(-1/2)$ line---error $\propto 1/\sqrt{n}$, so halving
  it costs four times the samples.}
  \label{fig:mc-integ}
\end{figure}
...
overlapping a ray and packs more tightly into the acceleration structure, which the reference reports
