...
\[
  ...
  \qquad \bm{x}_i \sim p .
\]
The estimator is unbiased for any $p$ whose support covers that of $f$, and---crucially---its
variance falls as $O(1/N)$, so the error falls as $O(1/\sqrt{N})$ \emph{independently of
...
contributions until paths are terminated by Russian roulette~\cite{ArvoKirk1990}. The media this
...
strongly forward, $g\approx 0.85$), $g=0$ is isotropic, and $g<0$ is backward. $p_{\mathrm{HG}}$ is
...
\]
which keeps the combined estimator's variance close to that of the best individual strategy.
...
extinction field is modelled as a weighted sum of $K$ anisotropic Gaussian kernels,
\[
