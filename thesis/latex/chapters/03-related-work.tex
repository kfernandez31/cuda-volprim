...
avoiding the popping that sort-order changes introduce. The goal---eliminating the sort---is the same
one this thesis pursues, but in the complementary setting of a physically based volumetric \emph{path
tracer} rather than a rasteriser, where the sort is removed by the decomposition-tracking argument
above instead of by stochastic blending.
...
on the table for a single-primitive production renderer: JIT kernel construction adds launch
overhead, the system is oriented toward single-frame and inverse-rendering workflows, and the
generated CUDA offers limited room for the fine-grained, primitive-specific optimisation this thesis
applies. These gaps motivate a bespoke CUDA/OptiX implementation. Mitsuba nonetheless remains
