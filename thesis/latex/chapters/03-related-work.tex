...
popping characteristic of splatting (\Cref{fig:flicker}). Both limitations point toward rendering the
same primitives by ray tracing instead of rasterisation.

\begin{figure}[htp]
  \centering
  \includegraphics[width=11cm]{images/flicker.png}
  \caption{Flickering and popping artefacts of rasterised Gaussian splatting, caused by the
  view-dependent global depth sort changing order discontinuously as the camera moves.}
  \label{fig:flicker}
\end{figure}
...
on the table for a bespoke production renderer: JIT kernel construction adds a per-process startup cost
(measured here at ${\approx}0.8$\,s warm and ${\approx}2.2$\,s cold, versus ${\approx}0.4$\,s for this
renderer's precompiled OptiX-IR pipeline---modest per run, but it compounds across the many short
launches of a parameter sweep); the
