...
the segment-marching, boundary-sorting, and root-finding that the reference uses for the scatter
...
algorithmic win (volumetric product-RIS direct lighting) and a rigorously characterised ledger of
negative results, and that closed the initial $\sim\!5\times$ equal-quality gap and overtook the
...
\begin{itemize}
  ...
  for the target assets at compile time; a substantially denser scene requires re-sizing. The runtime
  overflow counter \emph{detects} an undersized cap but does not fix it---the proper remedy is the
  pre-sizing tool noted below.
  ...
  used throughout.
  \item \textbf{Coloured albedo.} A small channel-dependent residual remains in RGB-albedo rendering.
\end{itemize}
...
re-measuring there. Third, an \emph{offline cap-estimation tool} that predicts a scene's worst-case
primitive overlap from its geometry, so the per-ray buffers can be sized correctly for any asset without
trial-and-error recompiles---turning the current cloud-tuned caps into a per-asset guarantee. Path
guiding, deferred here for want of a cheap predictive test (\Cref{sec:autopsies}), would also merit a
revisit with a full offline oracle.
