...
the sampler's sequential scan over distances touches only the \SI{4}{\byte} distance array, halving its
cache footprint. It is a per-ray buffer held in fast local memory
...
small convergence-stable residual at heavy overlap, where the independence assumption is stressed---a
double-precision $\operatorname{erf}^{-1}$ test rules out finite precision as the cause). Unlike the delta/null-tracking that decomposition tracking normally requires, no majorant
...
\paragraph{Complexity.} Per scattering bounce the work is $O(N + A + H)$: an $O(N)$ scan to maintain the
...
\emph{runtime} flags, while adaptive sampling and a fast \texttt{erf} (a polynomial approximation to $\operatorname{erf}$,
