...
SPP=1024
M=tools/refs/with_jorge_mitsuba.sh

bias(){ $PY tools/refs/furnace_check.py "$1" 1.0 2>&1 | grep -E "mean=|=>"; }
...
SG_ALBEDO=1.0 SG_ENV=white_constant SG_NEE=0 SG_SIGMA=6 SG_SPP=$SPP \
  $M $PY tools/refs/render_single_gaussian_via_prb.py 2>&1 | grep -E "wrote|mean" | tee -a "$LOG"
