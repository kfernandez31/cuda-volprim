...
def collect(arm):
    if arm == "ours":
        ...
        gt_files = [f"{OUT}/gt/gabor_nee_meadow_spp2048_seed{s}.exr" for s in (0, 1)]
        def rung_files(spp):
            return [f"{OUT}/ladder/gabor_nee_meadow_spp{spp}_seed{s}.exr" for s in (2, 3, 4, 5)]
...
def main():
    ...
    with open(f"{OUT}/convergence.csv", "w") as fh:
    ...
    styles = {"ours": dict(color="#1f77b4", marker="o", label="ours (MIS)"),
    ...
    ax.set_title("Cloud (meadow) convergence — each arm to its own GT")
    ax.grid(True, which="both", alpha=0.25); ax.legend(fontsize=8, framealpha=0.9)
    fig.tight_layout()
    fig.savefig(f"{OUT}/convergence.pdf"); fig.savefig(f"{OUT}/convergence.png", dpi=140)
