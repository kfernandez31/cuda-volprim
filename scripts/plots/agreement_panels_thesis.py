...
def tonemap(rgb, ex):
    a = np.clip(lum(rgb) * ex, 0, 1) ** (1 / 2.2)   # display gamma: linear values read near-black
...
rows = []
# meadow
mo = load_avg("results/campaign/cloud_conv/ours_spp1024_seed*.exr")
mn = load_avg(f"{OUT}/gt_fix5/gabor_nee_meadow_spp2048_seed*.exr")
...
cn = load_avg(f"{OUT}/constenv_fix5/gabor_nee_white_constant_spp256_seed0.exr")
bo, bg = lum(co)[:60, :60].mean(), lum(cn)[:60, :60].mean()
rows.append(("constant (uniform)", co, cn, bg / bo))

# crop to the cloud bounding box so the subject fills the frame (pad 20 px)
_lum_ref = lum(mn)
_bg = np.median(_lum_ref[:40, :40])
_mask = np.abs(_lum_ref - _bg) > 0.02 * max(_bg, 1e-3)
_r = np.where(_mask.any(1))[0]; _c = np.where(_mask.any(0))[0]
R0, R1 = max(_r[0]-20, 0), min(_r[-1]+20, _lum_ref.shape[0])
C0, C1 = max(_c[0]-20, 0), min(_c[-1]+20, _lum_ref.shape[1])
def crop(img):
...
for i, (title, o, n, norm) in enumerate(rows):
    ...
    axs[i, 0].imshow(crop(tonemap(o, ex))); axs[i, 0].set_ylabel(title, fontsize=11)
    axs[i, 0].set_title("this renderer" if i == 0 else "")
    axs[i, 1].imshow(crop(tonemap(n, ex)))
    axs[i, 1].set_title("reference" if i == 0 else "")
    axs[i, 2].imshow(crop(signed_rel(o, n, norm)))
    axs[i, 2].set_title("signed rel. diff (±20%)" if i == 0 else "")
    axs[i, 2].text(0.5, -0.09, f"mean ratio {ratio:.4f} (ours/reference)",
                   transform=axs[i, 2].transAxes, ha="center", fontsize=10)
    for j in range(3):
        axs[i, j].set_xticks([]); axs[i, j].set_yticks([])
pass
fig.tight_layout(rect=[0, 0, 1, 0.96])
fig.savefig(f"latex/figures/agreement_panels.pdf"); fig.savefig(f"latex/figures/agreement_panels.png", dpi=130)
