...
\Cref{ch:optimization} states this scope explicitly). Two supporting pillars carry this. The
...
whole-render cost. A cheaper, already-quantified target is the bounce-\num{0} containment scan of
\Cref{sec:results-scaling}: by the fitted cost model it takes ${\sim}5\,\%$ of frame
time on the small assets and ${\sim}12\,\%$ on the bunny, and its share grows with asset
size. Two untried fixes exist: precomputing the camera's containing set once per frame
(the bounce-\num{0} query point is shared by every sample), or a point query against the
acceleration structure for arbitrary origins. Either removes the renderer's one remaining
$O(N)$ per-sample term. Path guiding, deferred here for want of a cheap
...
resists the restructurings that help more conventional path tracers---knowledge that should
save the next implementer real time. The renderer, its test scenes, and the scripts behind
