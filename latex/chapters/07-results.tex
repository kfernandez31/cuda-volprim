...
fast paths against each other on equal terms. Both arms render the showcase ($900\times600$) under the
...
medium: its next-event estimator does not converge to the correct image, and its analog mode is
firefly-dominated under this illumination (diagnosis and revision timeline in
\Cref{sec:results-firefly}). A comparison against either shipped configuration would measure the
shipped code's limitations rather than estimator quality.
...
This comparison hinges on one convention, the \emph{reconstruction filter}. It is
...
(\Cref{ch:architecture}) is therefore a \emph{structural, throughput} simplification. Its
...
multiple-importance weight keeps two such strategies from counting the same light
...
the released source below. The current upstream revision no longer exhibits the over-count
(\emph{Upstream follow-up} below); its history contains the repair itself, a December 2025
revision that replaced the vertex-overlap density lookup with a direct fetch of the
primitive record, removing the zeroed read.
...
known core-level offset; a real discrepancy would leave a coherent pattern
(\Cref{sec:reference-problem}), and none is present beyond that offset. This bounds
...
clearest remaining optimisation target. Every measurement in this thesis was taken
before the following fix existed; it was implemented afterwards, on a separate branch,
so no reported number contains it. At bounce \num{0} the query point is the camera
position, shared by every sample, so a one-shot kernel precomputes the containing set
once per render with the same device predicate in the same order. Every output is
byte-identical to the unmodified renderer, and an A/B under the protocol of
\Cref{sec:setup}, at the \Cref{tab:asset-cost} configuration, shows whole frames
rendering $1.38\times$ faster on the cloud, $1.08\times$ on the tornado, $1.21\times$ on
the explosion, and $2.95\times$ on the bunny. The model's prediction was therefore a
\emph{lower} bound: it modelled the scan's arithmetic alone, and the measured excess is
consistent with the removed loop also relieving the register pressure that caps
occupancy (\Cref{sec:bottleneck}). The showcase headline is unaffected regardless: its
orthographic camera lies outside the fix's scope (\Cref{sec:future-work}).
