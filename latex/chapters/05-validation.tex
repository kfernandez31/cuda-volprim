...
fitted Gaussians at all. They show the fit's target: the source volume. The radiometric gap
is large. The bundled images average \num{0.55} over their \num{36} views (range
...
in both renderers at once. A stronger check removes the assumption. We render the same medium
by a route that never touches the Gaussian closed form, and ask whether the answer still
agrees. The Gaussian field is baked into a dense voxel grid in the renderer's own,
...
for it: the image depends only on transmittance, so no scattering variance enters. The grid
arms are delta-tracked at \num{32}\,spp, one seed each; the readouts below show the residual
is resolution-limited, not noise-limited. Through the opaque interior and the empty
background, the grid render matches the analytic render to numerical silence (median pixel
difference ${\sim}2\times10^{-5}$). One residual remains: a thin band at the silhouette.

The band is the grid's own error. A grid stores one density value per voxel. The Gaussians'
edge falloff is sharper than a voxel, so the grid blurs it; the analytic form evaluates it
exactly. Rays that graze the silhouette therefore accumulate slightly different optical
depth. The band follows the grid's discretisation. First, refining the grid shrinks the band
and moves the grid's image mean toward the analytic one.\footnote{From $128^3$ to $600^3$:
band RMSE $0.072\!\to\!0.057$; image means $0.394\!\to\!0.401$, analytic \num{0.416}.}
Second, adding samples barely moves it.\footnote{Quadrupling \num{32}\,spp to \num{128}\,spp
changes an intermediate $200^3$ grid's band RMSE from \num{0.0627} to \num{0.0613}.} The residual is
discretisation error. As the voxels shrink, the grid render converges to the
analytic one. The cross-check thus supports the renderer's correctness, and it also quantifies the
edge fidelity gained by the analytic primitive over a voxelised proxy.
...
\begin{figure}[htp]
  ...
  by Mitsuba's dense-grid delta-tracking path tracer. \emph{Right:} absolute difference at $5\times$ gain, gamma-mapped (white ${=}$
$|\Delta|\ge0.2$). The grid arm is delta-tracked at \num{32}\,spp, one seed. Interior and
background match to a median pixel difference of ${\sim}2\times10^{-5}$; the only residual is the silhouette
  band where the grid blurs the sharp Gaussian boundary, and it shrinks with grid resolution.}
  \label{fig:voxel-gt}
\end{figure}
...
Efficiency is a separate question, and it is compared only at \emph{equal quality}.
Monte~Carlo error falls as $O(1/\sqrt{N})$ (\Cref{sec:monte-carlo}), so the meaningful
quantity is the noise constant $k = \mathrm{RMSE}^2 \cdot N$. (Equivalently: inter-seed
variance $\times$ spp. The two coincide for unbiased estimators, as here; the headline
figures of \Cref{ch:results} use the variance form.) One renderer is faster than another only
if it reaches the same $k$ in less time. Raw frame times at a fixed sample count are reported
only alongside this equal-quality figure (\Cref{ch:results}).

Two controls guard this metric. \emph{Fireflies}---rare, very bright samples---inflate RMSE out of
proportion to perceived error, so robust statistics (high percentiles, clipped means) accompany the raw
numbers. The difference in firefly behaviour between the renderers is itself a result
...
\textbf{Result:} up to the active-set capacity, the renderer matches the reference for the
combined medium to $\sim\!10^{-4}$ in the mean. The closed-form additivity check is carried
by the collinear test, not by the plotted pair: $m$ identical Gaussians
stacked in place are exactly one $m$-fold-denser Gaussian, so additivity itself is the ground
truth. The test goes unplotted because its correct image is indistinguishable from the
single-Gaussian stage's. The plotted pair's recorded comparison is against Mitsuba.
...
plausible. The measurement still identified the mechanism. With $m$ stacked Gaussians sharing
...
reference's own errors, which is why the analytic check decides this stage.
...
level. The same brute-force arm also settles agreement in the \emph{optical-depth} domain,
...
both arms deterministic, so the comparison carries no Monte-Carlo noise at all). The absorption validation therefore has three stages, each adding one source of complexity over
the last. The single Gaussian is held to the analytic answer. The overlapping-primitives stage is
against the reference, with additivity checked analytically via the collinear test. The full cloud
is against the reference, plus the float64 brute-force check above settling the
silhouette band.
...
additivity checked analytically via the collinear test and the cloud's edge band settled by
...
computed on raw radiance. Difference panels: $\pm\num{5}\,\%$ full scale (conventions,
\Cref{sec:setup}).}
...
means. The comparison resolves effects at the $10^{-4}$ level. The
reference arm in these comparisons is the analog mode because it is the shipped code's unbiased configuration. The
reference's \emph{fast} (next-event) estimator is compared too, in its corrected
...
efficiency, and each arm only needs its own noise below the effect under test.} The method asks one question: how
different are the two mean images, and how different would their own residual noise alone make
them? The noise prediction comes from each arm's seed spread. Any measured difference above
...
\emph{feature-validation} exception; together with the overlap-region scattering residual
of \Cref{sec:scattering-ladder} and a third, it is one of three small residuals
documented as known limitations (\Cref{ch:conclusion}). The third is a ${+}1.8\times10^{-4}$
