...
specific: Monte~Carlo integration, the rendering equation and its
volumetric generalisation, the Gaussian kernel-mixture model, and hardware-accelerated
ray tracing. A recurring theme, picked up
...
\[
  \tau(t) = -\ln(1-\xi)
\]
for $t$, the \emph{free-flight} distance. In a general heterogeneous medium there is no
closed form for $\tau^{-1}$, and two distinct solution families exist: stochastic
\emph{tracking} (delta tracking and its variants, \Cref{sec:volumetric-mc}), which never
inverts $\tau$ at all, and \emph{regular tracking}, which marches a piecewise-analytic
medium boundary by boundary---the reference's approach, with a root-find inside
multi-primitive segments (\Cref{sec:dsyg}). Because each Gaussian primitive here has an analytic,
