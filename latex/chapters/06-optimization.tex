...
OptiX's built-in sphere routine. The measurement (\Cref{tab:icosphere}) decides
against the analytic sphere. The tessellated shell is \emph{faster at every tessellation level}: the analytic sphere pays $1.17$--$1.58\times$ in frame time for its exactness. The
...
finer tessellation buys worse accuracy at higher cost---consistent with the reference's
...
analytic sphere \emph{for correctness, at a now-quantified price}. Even the best shell
...
Performance is not only time; one memory result matters here: the megakernel's per-ray
state. The wavefront failure above turns on exactly that state: the per-ray bytes
(\Cref{sec:autopsies}), multiplied by the live ray count, are what overruns L2 and collapses
performance. That
is the
clearest evidence that keeping per-ray state register-resident is the design choice the
renderer's performance depends on. The acceleration structure itself is not where the budget goes. It is sub-megabyte
for the cloud and a few megabytes for the bunny, and it is dwarfed by the per-ray state. It is built
