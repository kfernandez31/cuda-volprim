...
THESIS_INLINE THESIS_HOST_DEVICE float max(float3 v) {
    return fmaxf(fmaxf(v.x, v.y), v.z);
}

THESIS_INLINE THESIS_HOST_DEVICE float sum(float3 v) {
    return v.x + v.y + v.z;
}

THESIS_INLINE THESIS_HOST_DEVICE bool almost_zero(float3 v, float eps = 1e-3f) {
    return max(v) < eps;
}
