...
public:
    ...
    THESIS_INLINE THESIS_HOST_DEVICE const T& operator[](size_t i) const noexcept { return data()[i]; }
...
struct DynamicVector : VectorBase<T, DynamicStorage<T>> {
    ...
    THESIS_HOST_DEVICE DynamicVector(T* ptr, size_t cap)
        : Base(DynamicStorage<T>{ptr, cap}) {}
};
