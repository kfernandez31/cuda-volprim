...
import gabor_bootstrap  # noqa: F401  (pip-3.8 shims for the corrected fork)
import volprim.integrators.volprim_prb

# --- corrected-fork adaptations (mirror gabor_cloud.py) ---
import volprim.integrators.volprim_prb as _vpprb
_orig_pt = _vpprb.primitive_tracing
def _pt_det(*a, **k):
    k.setdefault("stochastic_selection_criteria", "deterministic")
    k.setdefault("orientation_selection", "deterministic")
    return _orig_pt(*a, **k)
_vpprb.primitive_tracing = _pt_det
...
for _c in (_mi.Shape, _mi.ShapePtr):
    _install_alias(_c)
  # noqa: F401 (registers integrator)
...
OUT = Path(os.environ.get("SG_OUT", os.environ.get("OUT", "/tmp/mits_asset/0000.exr")))
...
dr.sync_thread()
t0 = time.time()
img = mi.render(scene, sensor=sensor, spp=SPP, seed=int(os.environ.get("SG_SEED", "0")))
dr.sync_thread()
