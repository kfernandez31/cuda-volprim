...
## Appendix A — A1 (per-step Rao-Blackwellization): full investigation

*Folded in from the former standalone `A1_INVESTIGATION.md` (branch `feature/a1-per-step-rb`,
2026-06-04). The headline framing below — "the flat-env gap is a NEE-vs-analog tradeoff" — was
subsequently **refined by §8.27** ("flat-env variance gap = collision vs track-length estimator",
whose title reads "corrects A1"); read §8.27 for the corrected conclusion. Retained here for the
detailed per-experiment evidence tables that §8.27 summarizes but does not reproduce. Scripts:
`tools/refs/a1_isolate.sh`, `tools/refs/a1_depth_sweep.sh`; seed renders under `renders/a1/`.*

### Premise (what A1 was supposed to be)
§8.5 measured CUDA ~2.85× noisier per sample than Mitsuba `volprim_prb` on the scattering cloud and
hypothesized the fix as **per-step RB**: "Mitsuba folds analytic segment transmittance into
throughput every bounce (`β*=seg_tr`); CUDA only does it at bounce 0 (`ENABLE_ANALYTIC_DIRECT`)."
...
**1. The literal premise is false — both estimators are analog.** Our `sample_scattering_event`
(argmin/ADT free-flight) and Jorge's `volprim_prb.sample_segment` + `primitive_tracing` both sample a
single scatter location from the transmittance CDF with a binary escape; Jorge's `β*=seg_tr` is the
free-flight distance-sampling accumulator, his `weight_rr` is RR compensation — **neither folds
transmittance into the path throughput in a way we don't.** SDTracking Thm 1 makes our argmin-min ≡
his combined CDF in distribution. → the named fix does not exist.

**2. The gap is flat-lighting-only; the showcase already wins** (FINDINGS §8.15).
...
**3. It's the per-vertex estimator, NOT argmin overlap** (`a1_isolate.sh`). Single Gaussian (NO
overlap), constant env, σ=2, alb 0.9, 6 seeds: CUDA **5.0× noisier** (0.00643 vs 0.00128). A
no-overlap scene still shows the gap → not an ADT/overlap effect.

**4. It's NOT the MIS env-IS overhead.** Rebuilt `ENABLE_MIS=false` (NEE only): noise 0.00607 — only
**1.06×** better. NEE-only is still **4.74×** noisier than Mitsuba-analog.

**5. The gap is in the converged multiple-scatter term** (`a1_depth_sweep.sh`). Per-seed noise vs max
depth (single-G const-env):
...
emitter) is **strictly worse than what CUDA already does**: CUDA's phase-IS NEE (strategy A) estimates
the same phase-sampled direct env but with an **analytic** shadow-ray transmittance, vs the
**stochastic** escape (`1 with prob T`) the continuation strategy would use. Same expectation, higher
variance. So that rewrite would *raise* variance on the direct term.

### Conclusion
- **Per-step RB (literal): impossible** — both estimators are analog.
- **The flat-env variance gap is a fundamental NEE-vs-analog tradeoff** (refined by §8.27 to
  collision-vs-track-length), confined to the multiple-scatter term in conservative + constant-emitter
  media. Analog wins there; NEE wins for real env maps. No universal estimator beats both.
...
- The only way to win the flat-env case is to switch the indirect term to analog there — abandoning the
  NEE advantage that wins env maps, i.e. a scene-adaptive hack helping only the non-showcase regime.

### Footnote: a small mean residual (separate from variance)
At convergence the means differ ~0.5% (CUDA 0.9947 vs Mitsuba 0.9893 at σ=2 single-G). Consistent with
the low-σ interior (§8.13, +1.8e-4) and RGB-albedo (§8.14, B +0.0046) residuals — a small
CUDA-slightly-brighter systematic; candidate causes the `MIN_THROUGHPUT` cull threshold (CUDA 1e-4 vs
Mitsuba `β<0.005`) or a minor multiple-scatter difference. Tracked as an OPEN item; not A1.
