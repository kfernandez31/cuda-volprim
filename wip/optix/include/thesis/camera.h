...
public:
    struct CameraSettings {
        ...
        vec3   lookfrom          = make_float3(0);         // Point camera is looking from
        vec3   lookat            = make_float3(0, 0, -1);  // Point camera is looking at
        vec3   vup               = make_float3(0, 1, 0);   // Camera-relative "up" direction
        ...
        , env_map(env_map_path)
    {
        ...
        const auto focal_length = length(settings.lookfrom - settings.lookat);
